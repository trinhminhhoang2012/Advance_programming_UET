...
void preorder_traversal(Node *v, vector<int> &order) {
	if (v!=NULL){
		order.push_back(v->data);
		preorder_traversal(v->left, order);
		preorder_traversal(v->right, order);
	}
}
...
void inorder_traversal(Node *v, vector<int> &order) {
	if (v!=NULL){
		inorder_traversal(v->left, order);
		order.push_back(v->data);
		inorder_traversal(v->right, order);
	}
}
...
void postorder_traversal(Node *v, vector<int> &order) {
	if (v!=NULL){
		postorder_traversal(v->left, order);
		postorder_traversal(v->right, order);
		order.push_back(v->data);
	}
}
...
int subtree_size(Node *v) {
	if (v==NULL) return 0;
	else return subtree_size(v->left) + subtree_size(v->right) + 1;
	//1 chi v
	//cac node con thuoc v->left
	//cac node con thuoc v->right
}
...
int is_descendant(Node *u, Node *v) {
	if (v->left == u) return 1;
	if (v->right == u) return 2;
	while (u->parent != NULL){
		u=u->parent;
		if (v->left == u) return 1;
		if (v->right == u) return 2;
